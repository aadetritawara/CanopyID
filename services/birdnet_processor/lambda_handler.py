...
def lambda_handler(event, context):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute(update_status_query, (current_job_id,))
            connection.commit()
        ...
        if connection:
            ...
            try:
                ...
                print(f"Failed to update job status to 'failed': {db_err}")
        try:
            r.publish(
                f"job:{current_job_id}",
                json.dumps(
                    {
                        "status": "failed",
                        "message": "We couldn't analyze this audio file. It might be corrupted or too short.",
                    }
                ),
            )
        except Exception as redis_err:
            print(f"Failed to publish error to Redis: {redis_err}")
    finally:
